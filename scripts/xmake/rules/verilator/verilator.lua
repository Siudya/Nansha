...
function config(target)
  ...
  local autogendir = target:values("verilator.compdir")
...
function build_cppfiles(target, batchjobs, sourcebatch, opt)
  local toolchain = assert(target:toolchain("verilator"), 'we need to set_toolchains("verilator") in target("%s")',
    target:name())
  local verilator = assert(toolchain:config("verilator"), "verilator not found!")
  local autogendir = assert(target:values("verilator.compdir"), "compilation directory must be assigned!")
  local targetname = target:name()
